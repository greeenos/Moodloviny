...
import { supportsImageInput } from 'shared/provider';
...
async function getContent(
  ...
  if (
    ...
    !supportsImageInput(config.model) ||
...
function loadPastHistory(): History | null {
  return JSON.parse(sessionStorage.moodlovinyHistory ?? 'null');
}
  ...
  return {
    ...
    saveResponse(response: string) {
      ...
      if (config.history) {
        ...
        sessionStorage.moodlovinyHistory = JSON.stringify(history);
      }
    }
  };
}
